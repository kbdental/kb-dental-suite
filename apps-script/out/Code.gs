...
// ── One-off maintenance: close the blank gap in the finance tabs ──────────
// The original mirror bug (see the note above saveReceipt) left a block of
// real receipts stranded far below the rest of the data in BOTH the entry tab
// and Working, with hundreds of blank rows in between. That gap is
// self-perpetuating: the clinic's Google Form appends below the last row
// containing ANYTHING, so every new response lands under the stranded block
// and the hole is preserved. Deleting the hole in the FY tabs does nothing —
// those read from Working, so it reappears on the next recalculation.
//
...
//   Date/Time — ARRAYFORMULA spill, which is what made getLastRow() lie and
//     caused this whole mess in the first place.
//   Checked   — a checkbox column. Google fills unticked boxes with a real
//     `false`, so every formatted-but-empty row would otherwise read as data
//     and the gap could never be closed.
function finDataWidth_(headers) {
  var width = headers.length;
  [finCol_(headers, ["Date"]), finCol_(headers, ["Time"]), finCol_(headers, ["Checked"])]
    .forEach(function(c) { if (c >= 0 && c < width) width = c; });
  return width;
}

// `false` counts as empty here for the checkbox reason above; a genuine value
// of 0 does not, since a zero-amount receipt is still a receipt.
function finCellEmpty_(v) {
  return v === "" || v === null || v === undefined || v === false;
}

function finScanTab_(tabName) {
  var sh = SpreadsheetApp.openById(getFinanceSheetId()).getSheetByName(tabName);
  if (!sh) throw new Error("The '" + tabName + "' tab was not found.");
  var lastRow = sh.getLastRow();
  var headers = sh.getRange(1, 1, 1, sh.getLastColumn()).getValues()[0].map(String);
  var width = finDataWidth_(headers);
  ...
  if (lastRow > 1 && width > 0) {
    ...
    for (var i = 1; i < vals.length; i++) {         // skip the header row
      if (!vals[i].every(finCellEmpty_)) filled.push(i + 1);
    }
  }
...
function reportFinanceGaps() {
  FIN_GAP_TABS.forEach(function(tabName) {
    ...
    Logger.log("Checked across columns 1..%s of %s (ignoring any Date/Time/Checked columns).",
